...
namespace Ben.LircSharp
{
    public class LircCommandParser
    {
        ...
        protected void ParseToken(string token)
        {
            try
            {
                // mkaltner: Added this code because it doesn't appear to handle commands FROM LIRC, just to LIRC.
                if (!string.IsNullOrEmpty(token) && token.Contains(' '))
                {
                    var parts = token.Split(' ');
                    if (parts.Length == 4 && parts[2].StartsWith("KEY_"))
                    {
                        var command = new LircCommand
                        {
                            Command = parts[2],
                            Data = null,
                            Succeeded = true
                ...
                // mkaltner: This code appears to only handle command responses
                switch (state)
                {
